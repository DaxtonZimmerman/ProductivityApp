...
function formatDeadline(task) {
  ...
  return task.time
    ? `${formattedDate} · ${date.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })}`
    : formattedDate;
}

function dayPartLabel(task) {
  const dayPart = getTimeOfDay(task.time);
  return dayPart === "anytime"
    ? "Anytime"
...
function renderTasks() {
  ...
  const visibleTasks = tasks
    .filter(
      (task) =>
        activeFilter === "all" || getTimeOfDay(task.time) === activeFilter,
    )
    .sort((first, second) => {
  ...
  if (!visibleTasks.length) {
    ...
    emptyState.className =
      "rounded-xl border border-dashed border-slate-300 px-4 py-8 text-center text-sm text-slate-500";
  ...
  visibleTasks.forEach((task) => {
    ...
    meta.className =
      "mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-500";
    ...
    deleteButton.className =
      "rounded-lg px-2 py-1 text-sm text-slate-400 hover:bg-rose-50 hover:text-rose-600";
...
taskForm.addEventListener("submit", (event) => {
  ...
  if (taskTime.value && !taskDate.value) {
    taskError.textContent =
      "Choose a date when adding a time so this task can appear on the calendar.";
